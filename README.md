...
- **Feed Explorer**: pick a stream, search, scroll; tweet embeds load as rows enter view. **Seen**: eye icon marks a tweet seen; **Hide seen** hides them from the main feed (open **Seen** in the sidebar to review or mark unseen). **Saved** tweets can include a **note** (stored in IndexedDB). **Multiple JSON uploads** are kept as separate **datasets**—switch with the **Dataset** control in the sidebar or toolbar; saved, seen, and trash are **per dataset**. Use **Remove current dataset** (sidebar) to delete one import and its marks. **Forever** (keep icon) stores a **copy of the tweet** in a separate IndexedDB database so pins **survive new uploads**; remove with **keep_off** in Forever or **Clear all Forever pins** on the Dashboard. **Trash** can be emptied or **purged from archive**. Toolbar: hold scroll buttons, density, embed theme, **back to top**; sidebar minimize uses **sessionStorage**.
- **Dashboard**: **Clear local data** removes **all** stored datasets and their marks from this browser (Forever pins are **not** cleared—use **Clear all Forever pins**). Then routes to upload.
- **Dashboard / Analytics**: summaries from the **active** dataset. Parsed data is stored in **IndexedDB** (no backend server).
